...
- **Muzzle flashes.** Each shot, yours or a visible player's, gives a short orange flash at the muzzle with a small star of light, a few sparks thrown forward, a brief light and a faint wisp of smoke. It's customizable without code: a game can put its own template (ParticleEmitters with a `Burst` count, fired from an attachment facing the way the gun fires, plus an optional PointLight named `Light`) in `Assets/Effects/Muzzle`, named after the weapon or `Default`.
