...
- **Reload animation.** Only for show, in the player's own view: drop the old magazine, slap in the new one, roll the gun onto its side and rack the slide, then come back up ready. It's timed as fractions of the reload, so it stretches to whatever `ReloadTime` a game sets. There's one reload time, empty or not; a game that wants empty reloads to be slower can mod that in. A game can change any part of it (poses, timings, hand paths, how springy it feels) per weapon with a ModuleScript at `Assets/Weapons/<Name>/Animation` that returns just the fields to change; the fields and their defaults are in `src/Client/WeaponAnimation.luau`.
