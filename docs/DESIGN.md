...
- **Sounds.** Client-side only. A game's weapon sounds go in `Assets/Weapons/<Name>/Sounds`, one Sound per cue named after it (`Fire`, `FireDistant`, `DryFire`, `Casing`, `Draw`, `MagazineOut`, `MagazineFloor`, `MagazineIn`, `SlideBack`, `SlideForward`, `ImpactWorld`, `ImpactBody`, `NearMiss`, `HitMarker`, `Headshot`), or a Folder of Sounds to pick one at random each time. Missing cues are silent, so the "systems only" download has no sound at all. Your own gun plays in your ears; other players' shots, impacts and near misses play where they happen. Draw and reload sounds are timed cues in the weapon's animation. Everything plays through the `OpenSRS` SoundGroup in SoundService, for one volume control.
